...
class Boat(object):
    ...
    def __init__(self, X, Y, x, y, color, outline, win):
        ...
        self.drawComandcenter(X, Y, x, y, color, outline, win)

    def drawBoat(self, X, Y, x, y, color, outline, win):
        boat = Polygon(Point(X+x,Y+y),Point(X + x + 175, Y + y), Point(X + x + 125, Y + y + 50),Point(X + x + 75,Y + y + 50),Point(X + x,Y + y),Point(X + x + 75, Y + y - 25),Point(X + x + 100, Y + y - 25),Point(X + x + 175,Y+ y))
        boat.setFill(color)
